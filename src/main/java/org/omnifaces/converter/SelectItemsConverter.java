...
/**
 * <p>
 * The <code>omnifaces.SelectItemsConverter</code> allows you to populate a selection component with complex Java model objects (entities) as value of
 * <code>&lt;f:selectItems&gt;</code> and have Faces convert those automatically back without the need to provide a custom converter which may need to do the
 * job based on possibly expensive service/DAO operations. This converter automatically converts based on the {@link #toString()} of the selected item.
 *
 * <h2>Usage</h2>
 * <p>
 ...
 * </pre>
 * <p>
 * By the way, <a href="https://stackoverflow.com/a/17343582/157882">you should also make sure that your entity has a good <code>equals()</code> and
 * <code>hashCode()</code> implementation</a>, otherwise Faces won't be able to set the right entity back in the model. Please note that this problem is in turn
 * unrelated to the <code>SelectItemsConverter</code>, you would have faced the same problem when using any other converter.
 *
 * <h2>If your entity can't have a good <code>toString()</code> implementation</h2>
 * <p>
 * However, if the entity doesn't have a <code>toString()</code> implementation (and thus relies on the default <code>fqn@hashcode</code> implementation), or
 ...
 * </pre>
 * <p>
 * Again, you do <strong>not</strong> need to override the <code>getAsObject()</code> method which would only need to perform possibly expensive service/DAO
 * operations. The <code>SelectItemsConverter</code> base converter will already do it automatically based on the available items and the
 * <code>getAsString()</code> implementation.
 * <p>
 * An alternative is to switch to {@link SelectItemsIndexConverter}, which will convert based on the position (index) of the selected item in the list instead
 * of the {@link #toString()} of the selected item.
 *
 * @author Arjan Tijms
