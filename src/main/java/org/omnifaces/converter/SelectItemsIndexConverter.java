...
/**
 * <p>
 * The <code>omnifaces.SelectItemsIndexConverter</code> is a variant of the {@link SelectItemsConverter} which automatically converts based on the position
 * (index) of the selected item in the list instead of the {@link #toString()} of the selected item.
 *
 * <h2>Usage</h2>
 * <p>
 ...
 * <li>No need to extend the {@link SelectItemsConverter} when {@link #toString()} method of the object cannot be used.</li>
 ...
 * <ul>
 * <li>The "Validation Error: value is not valid" will never occur anymore for the case that the available select items has incompatibly changed during the
 * postback due to a developer's mistake. The developer should make absolutely sure that exactly the same list is preserved on postback (e.g. by making it a
 * property of a view scoped or broader scoped bean).</li>
 * </ul>
 *
