...
/**
 * <p>
 * The {@link ResetInputAjaxActionListener} will reset input fields which are not executed during ajax submit, but which are rendered/updated during ajax
 ...
 * <ul>
 * <li>When Faces validation succeeds for a particular input component during the validations phase, then the submitted value is set to <code>null</code> and
 * the validated value is set as local value of the input component.
 * <li>When Faces validation fails for a particular input component during the validations phase, then the submitted value is kept in the input component.
 * <li>When at least one input component is invalid after the validations phase, then Faces will not update the model values for any of the input components.
 * Faces will directly proceed to render response phase.
 * <li>When Faces renders input components, then it will first test if the submitted value is not <code>null</code> and then display it, else if the local value
 * is not <code>null</code> and then display it, else it will display the model value.
 * <li>As long as you're interacting with the same Faces view, you're dealing with the same component state.
 * </ul>
 * <p>
 * So, when the validation has failed for a particular form submit and you happen to need to update the values of input fields by a different ajax action or
 * even a different ajax form (e.g. populating a field depending on a dropdown selection or the result of some modal dialog form, etc), then you basically need
 * to reset the target input components in order to get Faces to display the model value which was edited during invoke action. Otherwise Faces will still
 * display its local value as it was during the validation failure and keep them in an invalidated state.
 * <p>
 ...
 * any ways to register some kind of {@link AjaxBehaviorListener} in an application wide basis, let alone on a per <code>&lt;f:ajax&gt;</code> tag basis, so
 * that it also get applied to ajax actions in <code>UIInput</code> components. There are ways with help of {@link SystemEventListener}, but it ended up to be
 * too clumsy.
 *
 * <p>
 * <strong>See also</strong>: <br>
 * <a href="https://github.com/jakartaee/faces/issues/1060">Faces spec issue 1060</a>
 *
...
public class ResetInputAjaxActionListener extends DefaultPhaseListener implements ActionListener {
    ...
    /**
     * Construct a new reset input ajax action listener. This constructor will be used when specifying the action listener by
     * <code>&lt;f:actionListener&gt;</code> or when registering as <code>&lt;phase-listener&gt;</code> in <code>faces-config.xml</code>.
     */
