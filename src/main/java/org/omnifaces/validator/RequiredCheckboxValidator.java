...
/**
 * <p>
 * The <code>omnifaces.RequiredCheckboxValidator</code> is intented to solve a peculiar problem with <code>required="true"</code> attribute of
 * {@link UISelectBoolean} components like <code>&lt;h:selectBooleanCheckbox&gt;</code>. If you want to require the user to tick the desired checkbox, you would
 * expect that setting <code>required="true"</code> is sufficient. But it is not, the validation wil always pass.
 * <p>
 * As for every other {@link UIInput} component the default <code>required="true"</code> validator would only check if the value is actually filled and been
 * sent to the server side, i.e. the value is not null nor empty. In case of a <code>&lt;h:selectBooleanCheckbox&gt;</code>, which accepts <code>Boolean</code>
 * or <code>boolean</code> properties only, EL will coerce the unchecked value to <code>Boolean.FALSE</code> during apply request values phase right before
 * validations phase. This value is not <code>null</code> nor empty! Thus, the required attribute of the <code>&lt;h:selectBooleanCheckbox&gt;</code> is fairly
 * pointless. It would always pass the validation and thus never display the desired required message in case of an unticked checkbox.
 *
 * <h2>Usage</h2>
 * <p>
 ...
 * </pre>
 * <p>
 * The validator will use the message as specified in <code>requiredMessage</code>. If it's absent, then it will use the default required message as specified
 * in custom <code>&lt;message-bundle&gt;</code> in <code>faces-config.xml</code>. If it's absent, then it will default to <blockquote>{0}: a tick is
 * required"</blockquote>
 *
...
public class RequiredCheckboxValidator implements Validator<Boolean> {
    ...
    public void validate(FacesContext context, UIComponent component, Boolean value) {
        if (!(component instanceof UISelectBoolean)) {
            throw new IllegalArgumentException(ERROR_WRONG_COMPONENT.formatted(component.getClass().getName()));
        }
