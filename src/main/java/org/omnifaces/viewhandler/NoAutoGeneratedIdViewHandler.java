			...
			if (value != null && "id".equals(name)) {
				...
				if (id.startsWith(UNIQUE_ID_PREFIX) || id.contains(intermediateIdPrefix)) {
					...
					if (!(component instanceof UIViewRoot)) { // Skip viewstate hidden inputs.
						if (component == null) {
							logger.warning(format(ERROR_AUTO_GENERATED_ID_ENCOUNTERED, id, "<null>"));
						}
						else {
							String componentType = component.getClass().getName() + ":" + component.getRendererType();
							throw new IllegalStateException(format(ERROR_AUTO_GENERATED_ID_ENCOUNTERED, id, componentType));
						}
					}
				}
			}
