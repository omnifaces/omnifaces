...
/**
 * <p>
 * This {@link ViewHandler} once installed will during development stage throw an {@link IllegalStateException} whenever an automatically generated Faces
 * component ID (<code>j_id...</code>) is encountered in the rendered output. This has various advantages:
 * <ul>
 * <li>Keep the HTML output free of autogenerated Faces component IDs.
 ...
 * </ul>
 * <p>
 * Note that this does not check every component for its ID directly, but instead checks the {@link ResponseWriter} for writes to the "id" attribute. Components
 * that write their markup in any other way won't be checked and will thus slip through.
 * <p>
 * Additionally, since version 3.0 it will also log a warning line when <code>prependId="false"</code> is encountered. This is considered bad practice. See also
...
public class NoAutoGeneratedIdViewHandler extends ViewHandlerWrapper {
    ...
    /**
     * This response writer throws an {@link IllegalStateException} when an attribute with name "id" is written with a non-null value which starts with
     * {@link UIViewRoot#UNIQUE_ID_PREFIX} or contains an intermediate.
     *
     * @since 2.0
     * @author Arjan Tijms
     */
    public static class NoAutoGeneratedIdResponseWriter extends ResponseWriterWrapper {
        ...
        public void startElement(String name, UIComponent component) throws IOException {
            if (component != null && FALSE.equals(component.getAttributes().get("prependId"))) {
                logger.warning(() -> WARNING_PREPENDID_FALSE_ENCOUNTERED.formatted(getComponentType(component)));
            }
        ...
        public void writeAttribute(String name, Object value, String property) throws IOException {
            if (value != null && "id".equals(name)) {
                checkAutoGeneratedId(value.toString());
            }

            super.writeAttribute(name, value, property);
        }

        private void checkAutoGeneratedId(String value) {
            if (!value.startsWith(UNIQUE_ID_PREFIX) && !value.contains(intermediateIdPrefix)) {
                return;
            }

            var end = value.indexOf(separatorChar, value.indexOf(UNIQUE_ID_PREFIX));
            var id = end > 0 ? value.substring(0, end) : value;
            var context = Faces.getContext();
            var component = findComponentRelatively(context, Utils.<UIComponent>coalesce(getCurrentComponent(context), context.getViewRoot()), id);

            if (component instanceof UIViewRoot) { // Skip viewstate hidden inputs.
                return;
            }

            var message = ERROR_AUTO_GENERATED_ID_ENCOUNTERED.formatted(id, getComponentType(component));
