...
/**
 * <p>
 * This configuration interface parses the <code>/WEB-INF/web.xml</code> and all <code>/META-INF/web-fragment</code> files found in the classpath and offers
...
public interface WebXml {
    ...
    /**
     * Find for the given exception the right error page location. Exception types are matched as per Servlet 3.0 specification 10.9.2 with the exception that
     * the given exception is already unwrapped:
     * <ul>
     * <li>Make a pass through all specific exception types. If a match is found in the exception class hierarchy, use its location. The closest match in the
     * class hierarchy wins.
     * <li>Else use the default error page location, which can be either the java.lang.Throwable or HTTP 500 or default one.
     * </ul>
     *
    ...
    /**
     * Returns <code>true</code> if access to the given URL is allowed for the given role. URL patterns are matched as per Servlet 3.0 specification 12.1:
     * <ul>
     * <li>Make a first pass through all URL patterns. If an exact match is found, then check the role on it.
     * <li>Else make a recursive pass through all prefix URL patterns, stepping down the URL one directory at a time, trying to find the longest path match. If
     * it is found, then check the role on it.
     * <li>Else if the last segment in the URL path contains an extension, then make a last pass through all suffix URL patterns. If a match is found, then
     * check the role on it.
    ...
    /**
     * Returns a mapping of all error page locations by exception type. The default location is identified by <code>null</code> key.
     *
    ...
    /**
     * Returns a mapping of all security constraint URL patterns and the associated roles in the declared order. If the roles is <code>null</code>, then it
     * means that no auth constraint is been set (i.e. the resource is publicly accessible). If the roles is empty, then it means that an empty auth constraint
     * is been set (i.e. the resource is in no way accessible).
     *
