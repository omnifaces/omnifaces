 ...
 * </pre>
 * <p>
 * Note: the iteration is by design completely stateless. It's therefore not recommended to nest form components inside the <code>&lt;o:messages&gt;</code>
 * component. It should be used for pure output only, like as the standard <code>&lt;h:messages&gt;</code>. Plain output links are however no problem. Also note
 * that the <code>message</code> and <code>escape</code> attributes have in this case no effect. With a single message, there's no point of iteration. As to
 * escaping, just use <code>&lt;h:outputText escape="false"&gt;</code> the usual way.
 *
 * <h2>Design notice</h2>
 * <p>
 * The component class is named <code>OmniMessages</code> instead of <code>Messages</code> to avoid confusion with the {@link Messages} utility class.
 *
...
public class OmniMessages extends HtmlMessages {
    ...
    /**
     * Construct a new {@link OmniMessages} component whereby the renderer type is set to {@link MessagesRenderer#RENDERER_TYPE}.
     */
    public OmniMessages() {
