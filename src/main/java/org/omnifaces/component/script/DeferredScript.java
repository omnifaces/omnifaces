...
/**
 * <p>
 * The <code>&lt;o:deferredScript&gt;</code> is a component based on the standard <code>&lt;h:outputScript&gt;</code> which defers the loading of the given
 * script resource to the window load event. In other words, the given script resource is only loaded when the window is really finished with loading. So, the
 * enduser can start working with the webpage without waiting for the additional scripts to be loaded. Usually, it are those kind of scripts which are just for
 * progressive enhancement and thus not essential for the functioning of the webpage.
 * <p>
 ...
 * </pre>
 * <p>
 * You can use the optional <code>onbegin</code>, <code>onsuccess</code> and <code>onerror</code> attributes to declare JavaScript code which needs to be
 * executed respectively right before the script is loaded, right after the script is successfully loaded, and/or when the script loading failed.
 *
...
public class DeferredScript extends ScriptFamily {
    ...
    /**
     * Move this component to body using {@link #moveToBody(ComponentSystemEvent)}. If successfully moved, then set the script resource as rendered, so that
     * Faces won't auto-include it.
     */
    @Override
