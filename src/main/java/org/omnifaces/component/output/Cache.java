...
/**
 * <p>
 * The <code>&lt;o:cache&gt;</code> component allows to cache a fragment of rendered markup. The first request for a page that has this component on it will
 * cause this markup to be put into the cache. Then for subsequent requests the cached content is used directly and none of the components, backing beans and
 * services that were used to generate this content in the first place will be consulted.
 * <p>
 * Caching can take place in application scope, or in session scope. For individual fragments a time can be specified for which the cached content is valid.
 * After this time is elapsed, the very first request to the page containing the cache component in question will cause new content to be rendered and put into
 * the cache. A default time can be set per scope in web.xml.
 * <p>
 * For each scope a maximum capacity can be set. If the capacity for that scope is exceeded, an element will be removed following a least recently used policy
 * (LRU).
 * <p>
 * Via a cache provider mechanism an alternative cache implementation can be configured in web.xml. The default cache is based on {@link LruCache}.
 *
 * <h2>Setting a custom caching provider</h2>
 * <p>
 * A custom caching provider can be set by using the <code>org.omnifaces.CACHE_PROVIDER</code> context parameter in web.xml to point to an implementation of
 * <code>org.omnifaces.component.output.cache.CacheProvider</code>. For example:
 * 
 * <pre>
 * &lt;context-param&gt;
 ...
 * </pre>
 * <p>
 * The default provider, <code>org.omnifaces.component.output.cache.DefaultCacheProvider</code> can be used as an example.
 *
 * <h2>Global settings</h2>
 * <p>
 * For the default provider, the maximum capacity and the default time to live can be specified for the supported scopes "session" and "application". If the
 * maximum capacity is reached, an entry will be evicted following a least recently used policy. The default time to live specifies for how long entries are
 * considered to be valid. A value for the <code>time</code> attribute on this component will override this global default time to live. The following context
 * parameters can be used in web.xml:
 * <table>
 * <caption>All available context parameters</caption>
 * <tr>
 * <td class="colFirst"><code>org.omnifaces.CACHE_SETTING_APPLICATION_MAX_CAPACITY</code></td>
 * <td>Sets the maximum number of elements that will be stored per web module (application scope). Default: no limit</td>
 * </tr>
 * <tr>
 * <td class="colFirst"><code>org.omnifaces.CACHE_SETTING_SESSION_MAX_CAPACITY</code></td>
 * <td>Sets the maximum number of elements that will be stored per session. Default: no limit.</td>
 * </tr>
 * <tr>
 * <td class="colFirst"><code>org.omnifaces.CACHE_SETTING_APPLICATION_TTL</code></td>
 * <td>Sets the maximum amount of time in seconds that cached content is valid for the application scope. Can be overriden by individal cache components.
 * Default: no limit.</td>
 * </tr>
 * <tr>
 * <td class="colFirst"><code>org.omnifaces.CACHE_SETTING_SESSION_TTL</code></td>
 * <td>Sets the maximum amount of time in seconds that cached content is valid for the session scope. Can be overriden by individal cache components. Default:
 * no limit.</td>
 * </tr>
 * <tr>
 * <td class="colFirst"><code>org.omnifaces.CACHE_INSTALL_BUFFER_FILTER</code></td>
 * <td>Boolean that when <code>true</code> installs a Servlet Filter (Servlet 3.0+ only) that works in conjunction with the <code>useBuffer</code> attribute of
 * the Cache component to enable an alternative way to grab the content that needs to be cached. This is a convenience setting that is a short-cut for
 * installing the <code>org.omnifaces.servlet.BufferedHttpServletResponse</code> filter manually. If more finegrained control is needed regarding which place in
 * the filter chain the filter appears and which resources it exactly filters, this setting should not be used and the mentioned filter should be manually
 * configured. Default: <code>false</code>.</td>
 * </tr>
 * </table>
 *
 * @since 1.1
...
public class Cache extends OutputFamily {
    ...
    private static final String ERROR_NO_BUFFERED_RESPONSE = "No buffered response found in request, but 'useBuffer' set to true. Check setting the '%s' context parameter or installing the '%s' filter manually."
        .formatted(CacheInitializer.CACHE_INSTALL_BUFFER_FILTER, OnDemandResponseBufferFilter.class);
     ...
     * <p>
     * While auto-generated keys can be convenient, note that in the face of dynamic behavior on a view the id of a component and thus the cache key can change
     * in ways that are difficult to predict.
     * <p>
     * Keys are relative to the scope for which they are defined, meaning a key "foo" for the a session scoped cache will not conflict with a key of the same
     * name for an application scoped cache.
     * 
     ...
     * <p>
     * The supported scopes depent on the caching provider that is installed via the <code>org.omnifaces.CACHE_PROVIDER</code>. If no such provider is
     * installed, a default one is used that supports scopes "session" and "application".
     * <p>
    ...
    /**
     * Optional amount of time in seconds for which the cached content is valid (TTL). This is counted from the moment content is actually added to the cache.
     * If no time is provided the content will be subject to the global cache settings, and in absence of these are subject to the behavior of the cache
     * implementation that is used. The default cache implementation will simply cache indefinitely.
     * <p>
     * Whether the content is actually removed from the cache (to preserve memory) after the given time has elapsed is dependend on the actual cache
     * implementation that is used. The default cache implementation will <strong>NOT</strong> do this automatically, but will instead remove it only when the
     * cache item is being accessed again.
     * <p>
     * Following the above, new content will only be inserted into the cache following a page request. A time of e.g. <code>30</code> <strong>will not</strong>
     * cause new content to be inserted into the cache at <code>30</code> seconds intervals.
     * <p>
     * Note that this component <strong>does not</strong> support a cache loader and locking mechanism. This means after content times out, several simultaneous
     * page requests may render the same content and it's undetermined which of those will end up being cached.
     * 
     ...
     * <p>
     * Normally this content is obtained by replacing the so-called response writer when the parent Cache component delegates rendering to its children.
     * However, in some cases (like <code>h:form</code>) there is an amount of post-processing being done on the response outside the context of this parent -
     * child delegation.
     * <p>
     * Via this switch, the full response is buffered if the cache doesn't contain content for this component and special markers are inserted surrounding the
     * children's rendering. Afterwards, the content between the markers (if any) is extracted and inserted into the cache. Note that the full response is only
     * buffered incase there's no value in the cache. For all other requests this buffering will not happen.
     * <p>
     * Since this is clearly a more resource intensive and invasive method to grab content, it's not enabled by default. In addition to setting this attribute
     * to <code>true</code>, the <code>org.omnifaces.servlet.BufferedHttpServletResponse</code> Servlet Filter needs to be configured to filter the Faces
     * Servlet (or alternatively just the pages for which the buffering method should be used).
     * 
     ...
     * <p>
     * This attribute can be used to reset the cache, meaning that the first time the cache component is rendered again, it will re-render its children and will
     * cause any cached value expressions (via <code>o:cacheValue</code>) to be re-evaluated when its next referenced.
     * <p>
     * Note that this value has to remain true until the cache component is rendered, after that it should be set to false again otherwise the cached content
     * will be reset again at the next rendering.
     * 
     ...
     * <p>
     * This attribute can be used to disable the cache (temporarily). In disabled state the children will be rendered directly and cached content (if any) will
     * not be used, nor will the rendering outcome of the children be cached.
     * <p>
     * When the attribute is set to <code>false</code> again any content that was cached before the cache was disabled will be shown again if said content is
     * still available in the cache. The content that was rendered when the cache was disabled has no effect on any such previously cached content.
     * 
