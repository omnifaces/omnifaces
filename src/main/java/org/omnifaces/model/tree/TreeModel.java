...
public interface TreeModel<T> extends Iterable<TreeModel<T>>, Serializable {
    ...
    /**
     * Creates and adds a child tree node with the given wrapped data to the current tree node. It returns the newly created and added child tree node to ease
     * further building.
     *
    ...
    /**
     * Returns an unmodifiable list of all child tree nodes of the current tree node. Adding and removing elements is not supported on the list. Adding new
     * children should be done by the {@link #addChild(Object)} method on the tree node parent. Removing children should be done by the {@link #remove()} method
     * on the tree node itself.
     *
    ...
    /**
     * Returns the zero-based unique index of the current tree node. This is an underscore separated representation of the position of the node in the tree
     * hierarchy. The root node has index of <code>null</code>. The first child has index <code>0</code>. The second child of first child has index
     * <code>0_1</code>. The first child of second child of third child has index <code>2_1_0</code>.
     *
