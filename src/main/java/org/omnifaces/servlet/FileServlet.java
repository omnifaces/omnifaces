...
public abstract class FileServlet extends HttpServlet {
    ...
    private static final Pattern RANGE_PATTERN = Pattern.compile("^bytes=\\d*-\\d*(,\\d*-\\d*)*+$");
     ...
     * Set cache headers.
     */
    private static void setCacheHeaders(HttpServletRequest request, HttpServletResponse response, Resource resource, long expires) {
    ...
    private static List<Range> getRanges(HttpServletRequest request, Resource resource) {
        ...
        for (var rangeHeaderPart : rangeHeader.split("=")[1].split(",")) {
