...
/**
 * <p>
 * The <code>&lt;o:converter&gt;</code> is a taghandler that extends the standard <code>&lt;f:converter&gt;</code> tag family with support for deferred value
 ...
 * <a href="https://jakarta.ee/specifications/platform/9/apidocs/jakarta/faces/convert/package-summary.html">their javadocs</a>. First go to the javadoc of the
 * class of interest, then go to <code>CONVERTER_ID</code> in its field summary and finally click the Constant Field Values link to see the value.
 *
 * <h2>JSF 2.3 compatibility</h2>
 * <p>
 * The <code>&lt;o:converter&gt;</code> is currently not compatible with converters which are managed via the <code>managed=true</code> attribute set on the
 * {@link FacesConverter} annotation, at least not when using Mojarra. Internally, the converters are wrapped in another instance which doesn't have the needed
 * setter methods specified. In order to get them to work with <code>&lt;o:converter&gt;</code>, the <code>managed=true</code> attribute needs to be removed, so
 * that OmniFaces {@link ConverterManager} will automatically manage them.
 *
...
public class Converter extends ConverterHandler implements DeferredTagHandler {
    ...
    /**
     * Create a {@link jakarta.faces.convert.Converter} based on the <code>binding</code> and/or <code>converterId</code> attributes as per the standard Faces
    ...
    private static jakarta.faces.convert.Converter<Object> createInstance(
    ...
    protected static class DeferredConverter implements jakarta.faces.convert.Converter<Object>, Serializable {
         ...
         */
        public DeferredConverter(
            jakarta.faces.convert.Converter<Object> converter, ValueExpression binding, ValueExpression id,
            DeferredAttributes attributes
        )
        {
