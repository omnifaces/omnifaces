 ...
 * model does not represent <strong>all</strong> available rows of the {@link UIData} component (e.g. when there's means of lazy loading). In such case you need
 * to supply your own validator which queries the underlying data store.
 * <p>
...
public class ValidateUniqueColumn extends TagHandler implements ValueChangeListener {
    ...
    public void apply(FaceletContext context, UIComponent parent) throws IOException {
        ...
        if (!(parent instanceof UIInput)) {
            throw new IllegalArgumentException(ERROR_INVALID_PARENT.formatted(parent.getClass().getName()));
        }
    ...
    private static class UniqueColumnValueChecker implements VisitCallback {
        ...
        public VisitResult visit(VisitContext context, UIComponent target) {
            ...
            // Deliberately getValue() and not getLocalValue(): rows which were not submitted, because of pagination or
            // because the ajax request executed only a subset of the form, have no local value at all, so their value
            // must be obtained from the data model instead.
            if (
