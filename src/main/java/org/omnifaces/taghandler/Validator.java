...
/**
 * <p>
 * The <code>&lt;o:validator&gt;</code> is a taghandler that extends the standard <code>&lt;f:validator&gt;</code> tag family with support for deferred value
 ...
 * <a href="https://jakarta.ee/specifications/platform/9/apidocs/jakarta/faces/validator/package-summary.html">their javadocs</a>. First go to the javadoc of
 * the class of interest, then go to <code>VALIDATOR_ID</code> in its field summary and finally click the Constant Field Values link to see the value.
 * <p>
 ...
 * <p>
 * The <code>&lt;o:validator&gt;</code> is currently not compatible with validators which are managed via the <code>managed=true</code> attribute set on the
 * {@link FacesValidator} annotation, at least not when using Mojarra. Internally, the converters are wrapped in another instance which doesn't have the needed
 * setter methods specified. In order to get them to work with <code>&lt;o:validator&gt;</code>, the <code>managed=true</code> attribute needs to be removed, so
 * that OmniFaces {@link ValidatorManager} will automatically manage them.
 *
...
public class Validator extends ValidatorHandler implements DeferredTagHandler {
    ...
    @FacesAttribute(
        name = "for", description = "If present, this attribute refers to the value of one of the exposed attached objects within the composite component inside of which this tag is nested."
    )
    ...
    /**
     * Create a {@link jakarta.faces.validator.Validator} based on the <code>binding</code> and/or <code>validatorId</code> attributes as per the standard Faces
    ...
    private static jakarta.faces.validator.Validator<Object> createInstance(
        FacesContext facesContext, ELContext elContext, ValueExpression binding,
        ValueExpression id
    )
    {
    ...
    protected static class DeferredValidator implements jakarta.faces.validator.Validator<Object>, Serializable {
         ...
         */
        public DeferredValidator(
            jakarta.faces.validator.Validator<Object> validator, ValueExpression binding, ValueExpression id, ValueExpression disabled,
            ValueExpression message, DeferredAttributes attributes
        )
        {
            ...
            if (message != null) {
                ...
                if (validatorMessage != null) {
                    ...
                    throw new ValidatorException(
                        Messages.create(validatorMessage, label)
                            .detail(validatorMessage, label).error().get(),
                        e.getCause()
                    );
                }
            }
