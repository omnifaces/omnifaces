...
public class MassAttribute extends TagHandler {
        ...
        if (target != null) {
            ...
            for (var i = 0; i < classNames.size(); i++) {
                ...
                try {
                    ...
                    throw new IllegalArgumentException(ERROR_UNAVAILABLE_TARGET.formatted(className), e);
                }

                if (!UIComponent.class.isAssignableFrom(cls)) {
                    throw new IllegalArgumentException(ERROR_INVALID_TARGET.formatted(cls));
                }
    ...
    public void apply(FaceletContext context, UIComponent parent) throws IOException {
        var oldComponents = newSetFromMap(new IdentityHashMap<UIComponent, Boolean>());
        oldComponents.addAll(findComponentsInChildren(parent, UIComponent.class));
        nextHandler.apply(context, parent);
        var newComponents = findComponentsInChildren(parent, UIComponent.class);
