...
/**
 * <p>
 * The {@link ViewExpiredExceptionHandler} will suppress any {@link ViewExpiredException} and refresh the current page by redirecting to the current URL with
 * query string. Additionally, it will set a flash attribute indicating that the {@link ViewExpiredException} was handled by this exception handler.
 *
 * <h2>Installation</h2>
 * <p>
 ...
 * <h3>Note</h3>
 * <p>
 * It's your own responsibility to make sure that the end user gets some form of feedback as to why exactly the page is refreshed and any submitted input values
 * are lost. In order to check whether the previous request threw a {@link ViewExpiredException} which was handled by this exception handler, then you can use
 * {@link ViewExpiredExceptionHandler#wasViewExpired()} in managed beans and <code>#{flash['org.omnifaces.view_expired'] eq true}</code> in EL.
 * <p>
 * This approach will not work when the refresh in turn triggers yet another redirect elsewhere in the logic. In case you want to retain the condition for the
 * next request, then you need to ensure that the involved logic explicitly triggers {@link Flash#keep(String)} in order to keep the flash attribute for the
 * subsequent request. In the scope of OmniFaces, this is already taken care of by {@link Faces#redirect(String, Object...)} and derivates.
 *
 * @author Lenny Primak
...
public class ViewExpiredExceptionHandler extends ExceptionSuppressor {
    ...
    /**
     * The flash attribute name of a boolean value indicating that the previous request threw a {@link ViewExpiredException} which was handled by this exception
     * handler.
     */
