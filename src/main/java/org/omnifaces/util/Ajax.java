...
/**
 * <p>
 * Collection of utility methods for working with {@link PartialViewContext}. There are also shortcuts to the current {@link OmniPartialViewContext} instance.
 * <p>
 * This utility class allows an easy way of programmaticaly (from inside a managed bean method) specifying new client IDs which should be ajax-updated via
 * {@link #update(String...)}, also {@link UIData} rows or columns on specific index via {@link #updateRow(UIData, int)} and {@link #updateColumn(UIData, int)},
 * specifying callback scripts which should be executed on complete of the ajax response via {@link #oncomplete(String...)}, and loading new JavaScript
 * resources on complete of the ajax response via {@link #load(String, String)}.
 * <p>
 * It also supports adding arguments to the JavaScript scope via {@link #data(String, Object)}, {@link #data(Map)} and {@link #data(Object...)}. The added
 * arguments are during the "on complete" phase as a JSON object available by <code>OmniFaces.Ajax.data</code> in JavaScript context. The JSON object is encoded
 * by {@link Json#encode(Object)} which supports standard Java types {@link Boolean}, {@link Number}, {@link CharSequence} and {@link Date} arrays,
 ...
 * {@link Map}.
 * <p>
 * Since 5.4, each Faces ajax response also exposes {@link FacesContext#isValidationFailed()} to the client side as <code>OmniFaces.Ajax.validationFailed</code>
 * (boolean), so JavaScript can react to validation failures without server or DOM round-trips, e.g. <code>if (OmniFaces.Ajax.validationFailed) { ... }</code>.
 * <p>
...
public final class Ajax {
     ...
     * <code>:</code>. This method also supports the client ID keywords <code>@all</code>, <code>@form</code> and <code>@this</code> which respectively refers
    ...
    /**
     * Update the row of the given {@link UIData} component at the given zero-based row index. This will basically update all direct children of all
     * {@link UIColumn} components at the given row index.
     * <p>
    ...
    /**
     * Update the column of the given {@link UIData} component at the given zero-based column index. This will basically update all direct children of the
     * {@link UIColumn} component at the given column index in all rows. The column index is the physical column index and does not depend on whether one or
     * more columns is rendered or not (i.e. it is not necessarily the same column index as the enduser sees in the UI).
     * <p>
     * Note that the to-be-updated direct child of {@link UIColumn} must be a fullworthy Faces UI component which renders a concrete HTML element to the output,
     * so that JS/ajax can update it. So if you have due to design restrictions for example a <code>&lt;h:panelGroup rendered="..."&gt;</code> without an ID,
     * then you should give it an ID. This way it will render a <code>&lt;span id="..."&gt;</code> which is updateable by JS/ajax.
