...
/**
 * <p>
 * Collection of utility methods for the Faces API that are mainly shortcuts for obtaining stuff from the thread local {@link FacesContext}. In effects, it
 * 'flattens' the hierarchy of nested objects. Do note that using the hierarchy is actually a better software design practice, but can lead to verbose code.
 * <p>
 * Next to those oneliner delegate calls, there are also some helpful methods which eliminates multiline boilerplate code, such as {@link #getLocale()} which
 * returns sane fallback values, a more convenient {@link #redirect(String, Object...)} which automatically prepends the context path when the path does not
 * start with <code>/</code> and offers support for URL encoding of request parameters supplied by varargs argument, and several useful
 * {@link #sendFile(File, boolean)} methods which allows you to provide a {@link File}, <code>byte[]</code> or {@link InputStream} as a download to the client.
 *
 * <h2>Usage</h2>
 * <p>
 ...
 * <h2>FacesLocal</h2>
 * <p>
 * Note that there's normally a minor overhead in obtaining the thread local {@link FacesContext}. In case client code needs to call methods in this class
 * multiple times it's expected that performance will be slightly better if instead the {@link FacesContext} is obtained once and the required methods are
 * called on that, although the difference is practically negligible when used in modern server hardware.
 * <p>
 * In such case, consider using {@link FacesLocal} instead. The difference with {@link Faces} is that no one method of {@link FacesLocal} obtains the
 * {@link FacesContext} from the current thread by {@link FacesContext#getCurrentInstance()}. This job is up to the caller.
 *
 * <h2>#{faces} in EL</h2>
 * <p>
 * Since OmniFaces 2.6, all methods of {@link Faces} utility class which start with "get" or "is", and take no parameters, and return either <code>String</code>
 * or <code>boolean</code>, and are not related to response nor to session or flash (for which already implicit EL objects <code>#{session}</code> and
...
public final class Faces {
    ...
    /**
     * Sets the given faces context as current instance. Use this if you have a custom {@link FacesContextWrapper} which you'd like to (temporarily) use as the
     * current instance of the faces context.
     *
     ...
     * <p>
     * This method is an alternative for {@link Faces#getApplication()} for those situations where the {@link FacesContext} isn't available.
     *
     * @return The faces application singleton.
     */
    ...
    /**
     * Returns the project stage. This will return the <code>jakarta.faces.PROJECT_STAGE</code> context parameter in <code>web.xml</code>.
     *
    ...
    /**
     * Returns whether we're in development stage. This will be the case when the <code>jakarta.faces.PROJECT_STAGE</code> context parameter in
    ...
    /**
     * Returns whether we're in system test stage. This will be the case when the <code>jakarta.faces.PROJECT_STAGE</code> context parameter in
     * <code>web.xml</code> is set to <code>SystemTest</code>.
     * <p>
    ...
    /**
     * Returns whether we're in production stage. This will be the case when the <code>jakarta.faces.PROJECT_STAGE</code> context parameter in
     * <code>web.xml</code> is set to <code>Production</code>.
     * <p>
    ...
    /**
     * Determines and returns the faces servlet mapping used in the current request. If Faces is prefix mapped (e.g. <code>/faces/*</code>), then this returns
     * the whole path, with a leading slash (e.g. <code>/faces</code>). If Faces is suffix mapped (e.g. <code>*.xhtml</code>), then this returns the whole
     * extension (e.g. <code>.xhtml</code>). If there is no extension, then this falls back to {@link ViewHandler#DEFAULT_SUFFIX_PARAM_NAME}.
     * <p>
     ...
     *
     * @return <code>true</code> if the faces servlet mapping used in the current request is a prefix mapping, otherwise <code>false</code>.
     * @see #getMapping()
    ...
    /**
     * Returns whether the given faces servlet mapping is a prefix mapping. Use this method in preference to {@link #isPrefixMapping()} when you already have
     * obtained the mapping from {@link #getMapping()} so that the mapping won't be calculated twice.
     *
    ...
    /**
     * Signals Faces that the validations phase of the current request has failed. This can be invoked in any other phase than the validations phase. The value
     * can be read by {@link #isValidationFailed()} in Java and by <code>#{facesContext.validationFailed}</code> in EL.
     *
     * @throws NullPointerException When faces context is unavailable.
     ...
     *
     * @return <code>true</code> if the validations phase of the current request has failed, otherwise <code>false</code>.
     * @throws NullPointerException When faces context is unavailable.
    ...
    /**
     * Creates and returns a Faces converter associated with given object identifier. If the given identifier is an instance of string, then delegate to
     * {@link #createConverter(String)}. If the given identifier is an instance of class, then delegate to {@link #createConverter(Class)}. If the given
     * identifier is a concrete converter instance, then return it directly. If no converter instance can be associated, then return null.
     *
     * @param <T> The expected converter type.
     * @param identifier The Faces converter object identifier. This can be a string representing the converter ID, or a class representing the target type, or
     * a class representing the converter class, or even the converter instance itself.
    ...
    /**
     * Creates and returns a Faces converter associated with given string identifier. First use the identifier as converter ID in
     * {@link Application#createConverter(String)}. If that didn't return anything, then try to interpret the string identifier as class name and delegate to
     * {@link #createConverter(Class)}. If no converter instance can be associated, then return null.
     *
     * @param <T> The expected converter type.
    ...
    /**
     * Creates and returns a Faces converter associated with given class identifier. If the given identifier is not assignable to Converter.class, then use that
     * as target type in {@link Application#createConverter(Class)}. If the given identifier is assignable to Converter.class, and the {@link FacesConverter}
     * annotation is present, then instantiate it using CDI, else instantiate it using default constructor. If no converter instance can be associated, then
     * return null.
     *
     * @param <T> The expected converter type.
    ...
    /**
     * Creates and returns a Faces validator associated with given object identifier. If the given identifier is an instance of string, then delegate to
     * {@link #createValidator(String)}. If the given identifier is an instance of class, then delegate to {@link #createValidator(Class)}. If the given
     * identifier is a concrete validator instance, then return it directly. If no validator instance can be associated, then return null.
     *
     * @param <T> The expected validator type.
     * @param identifier The Faces validator object identifier. This can be a string representing the validator ID, or a class representing the validator class,
     * or even the validator instance itself.
    ...
    /**
     * Creates and returns a Faces validator associated with given string identifier. First use the identifier as validator ID in
     * {@link Application#createValidator(String)}. If that didn't return anything, then try to interpret the string identifier as class name and delegate to
     * {@link #createValidator(Class)}. If no validator instance can be associated, then return null.
     *
     * @param <T> The expected validator type.
    ...
    /**
     * Creates and returns a Faces validator associated with given class identifier. If the given identifier is assignable to Validator.class, and the
     * {@link FacesValidator} annotation is present, then instantiate it using CDI, else instantiate it using default constructor. If no validator instance can
     * be associated, then return null.
     *
     * @param <T> The expected validator type.
    ...
    /**
     * Sets the current view root to the given view ID. The view ID must start with a leading slash. If an invalid view ID is given, then the response will
    ...
    /**
     * Returns the base name of the current view, without extension, or <code>null</code> if there is no view. E.g. if the view ID is
     * <code>/path/to/some.xhtml</code>, then this will return <code>some</code>.
     * <p>
     ...
     * <p>
     * The current view ID is the view ID that's set for the view root that's associated with the current faces context. Or if there is none, then the current
     * view handler will be assumed, which is the view handler that's associated with the requested view.
     *
     ...
     * <li>If the current mapping is a suffix mapping and the given path ends with it, then replace it with the default Facelets suffix.
     * </ul>
     *
    ...
    /**
     * Returns the view parameters of the current view as a parameter map, or an empty map if there is no view. This is ready for usage in among others
     * {@link ViewHandler#getBookmarkableURL(FacesContext, String, Map, boolean)}.
     *
    ...
    /**
     * Returns the hash query string of the current view, or <code>null</code> if there is none. This is the part after the <code>#</code> in the request URL as
     * the enduser sees in browser address bar. This works only if the hash parameters are via <code>&lt;o:hashParam&gt;</code> registered in the view.
     *
    ...
    /**
     * Returns the metadata attribute of the given view ID associated with the given name. Note: this is not the same as the view scope, for that use
    ...
    /**
     * Returns the metadata attribute of the current view associated with the given name. Note: this is not the same as the view scope, for that use
     * {@link #getViewAttribute(String)}.
     *
     * @param <T> The expected return type.
     * @param name The metadata attribute name.
    ...
    /**
     * Returns the current locale. If the locale set in the Faces view root is not null, then return it. Else if the client preferred locale is not null and is
     * among supported locales, then return it. Else if the Faces default locale is not null, then return it. Else return the system default locale.
     *
    ...
    /**
     * Returns an unordered list of all supported locales on this application, with the default locale as the first item, if any. This will return an empty list
     * if there are no locales definied in <code>faces-config.xml</code>.
     *
     * @return An unordered list of all supported locales on this application, with the default locale as the first item, if any. If you need an ordered list,
     * use {@link FacesConfigXml#getSupportedLocales()} instead.
     * @throws NullPointerException When faces context is unavailable.
     * @see Application#getDefaultLocale()
     ...
     * @throws NullPointerException When faces context is unavailable.
     * @throws IllegalStateException When there is no view (i.e. when it is <code>null</code>). This can happen if the method is called at the wrong moment in
     * the Faces lifecycle, e.g. before the view has been restored/created.
    ...
    /**
     * Returns the application message bundle as identified by <code>&lt;message-bundle&gt;</code> in <code>faces-config.xml</code>. The instance is already
     * localized via {@link Faces#getLocale()}. If there is no <code>&lt;message-bundle&gt;</code>, then this method just returns <code>null</code>.
     *
     * @return The message bundle as identified by <code>&lt;message-bundle&gt;</code> in <code>faces-config.xml</code>.
     * @throws NullPointerException When faces context is unavailable.
     * @throws MissingResourceException When the <code>&lt;message-bundle&gt;</code> in <code>faces-config.xml</code> does not refer an existing resource in the
     * classpath.
    ...
    /**
     * Returns the application resource bundle as identified by the given <code>&lt;var&gt;</code> of the <code>&lt;resource-bundle&gt;</code> in
     * <code>faces-config.xml</code>. If there is no <code>&lt;resource-bundle&gt;</code> with the given <code>&lt;var&gt;</code>, then this method just returns
     * <code>null</code>.
     *
     * @param varName The value of the <code>&lt;var&gt;</code> which identifies the <code>&lt;resource-bundle&gt;</code> in <code>faces-config.xml</code>.
     * @return The application resource bundle as identified by the given <code>&lt;var&gt;</code> of the <code>&lt;resource-bundle&gt;</code> in
     * <code>faces-config.xml</code>.
     * @throws NullPointerException When faces context is unavailable.
     * @throws MissingResourceException When the <code>&lt;resource-bundle&gt;</code> as identified by the given <code>&lt;var&gt;</code> in
     * <code>faces-config.xml</code> does not refer an existing resource in the classpath.
    ...
    /**
     * Returns all application resource bundles registered as <code>&lt;resource-bundle&gt;</code> in <code>faces-config.xml</code>. If there are no resource
     * bundles registered, then this method just returns an empty map.
     *
     * @return all application resource bundles registered as <code>&lt;resource-bundle&gt;</code> in <code>faces-config.xml</code>.
     * <code>&lt;base-name&gt;</code> of the <code>&lt;resource-bundle&gt;</code> in <code>faces-config.xml</code>.
     * @throws NullPointerException When faces context is unavailable.
     * @throws MissingResourceException When the <code>&lt;resource-bundle&gt;</code> in <code>faces-config.xml</code> does not refer an existing resource in
     * the classpath.
     * @see Application#getResourceBundle(FacesContext, String)
     * @since 2.1
     */
    ...
    /**
     * Gets a string for the given key searching declared resource bundles, order by declaration in <code>faces-config.xml</code>. If the string is missing,
     * then this method returns <code>???key???</code>.
     *
     * @param key The bundle key.
     * @param params Since 4.6.2 The {@link MessageFormat} parameters, if any.
     * @return a string for the given key searching declared resource bundles, order by declaration in <code>faces-config.xml</code>.
     * @throws NullPointerException When faces context is unavailable.
     * @since 2.1
     */
    ...
    /**
     * Returns the concrete domain-relative URL to the current view with the given params URL-encoded in the query string and optionally include view parameters
     * as well. This URL can ultimately be used as redirect URL, or in <code>&lt;form action&gt;</code>, or in <code>&lt;a href&gt;</code>. Any parameter with
     * an empty name or value will be skipped. To skip empty view parameters as well, use <code>&lt;o:viewParam&gt;</code> instead.
     *
    ...
    /**
     * Returns the concrete domain-relative URL to the given view with the given params URL-encoded in the query string and optionally include view parameters
     * as well. This URL can ultimately be used as redirect URL, or in <code>&lt;form action&gt;</code>, or in <code>&lt;a href&gt;</code>. Any parameter with
     * an empty name or value will be skipped. To skip empty view parameters as well, use <code>&lt;o:viewParam&gt;</code> instead.
     *
     * @param viewId The view ID to create the bookmarkable URL for.
     * @param params The parameters to be URL-encoded in the query string. Can be <code>null</code>.
     * @param includeViewParams Whether the view parameters of the current view which are also declared in the target view should be included as well. Note thus
     * that this does not include the view parameters which are not declared in the target view!
    ...
    /**
     * Returns the Facelet attribute value associated with the given name. This basically returns the value of the <code>&lt;ui:param&gt;</code> which is been
     * declared inside the Facelet file, or is been passed into the Facelet file by e.g. an <code>&lt;ui:include&gt;</code>.
     *
     * @param <T> The expected return type.
    ...
    /**
     * Sets the Facelet attribute value associated with the given name. This basically does the same as an <code>&lt;ui:param&gt;</code> which is been declared
     * inside the Facelet file, or is been passed into the Facelet file by e.g. an <code>&lt;ui:include&gt;</code>.
     *
     * @param name The Facelet attribute name.
    ...
    /**
     * Returns whether the current request is an ajax request with partial rendering. That is, when it's an ajax request without <code>render="@all"</code>.
     * <p>
     * This is also available in EL as <code>#{faces.ajaxRequestWithPartialRendering}</code>.
     *
     * @return <code>true</code> for an ajax request with partial rendering, <code>false</code> an ajax request with <code>render="@all"</code> or a non-ajax
     * (synchronous) request.
     * @throws NullPointerException When faces context is unavailable.
     * @see PartialViewContext#isAjaxRequest()
    ...
    /**
     * Returns whether the current request is a postback. This not only delegates to {@link FacesContext#isPostback()} which checks the presence of
     * <code>jakarta.faces.ViewState</code> request parameter, but this also explicitly checks the HTTP request method. So this should exclude GET requests
     * having a <code>jakarta.faces.ViewState</code> request parameter in query string.
     * <p>
    ...
    /**
     * Returns the HTTP request parameter value associated with the given name and implicitly converted to given type using the Faces converter registered by
     * <code>forClass</code> on the given type. Note that empty strings are already implicitly converted to <code>null</code>.
     *
     * @param <T> The expected return type.
     * @param name The HTTP request parameter name.
    ...
    /**
     * Returns the HTTP request parameter value associated with the given name and converted using the given converter. Note that empty strings are already
     * implicitly converted to <code>null</code>.
     *
     * @param <T> The expected return type.
     * @param name The HTTP request parameter name.
    ...
    /**
     * Returns the HTTP request parameter value associated with the given name and convert it using the given converter, or else the supplied value if absent.
     ...
     * @return The HTTP request parameter value associated with the given name and convert it using the given converter, or else the supplied value if absent.
     * @throws ClassCastException When <code>T</code> is of wrong type.
    ...
    /**
     * Returns the HTTP request parameter values associated with the given name and implicitly convert it to given type using the Faces converter registered by
     * <code>forClass</code> on the given type.
     *
     * @param <T> The expected return type.
     * @param name The HTTP request parameter name.
     * @param type The converter <code>forClass</code> type.
    ...
    /**
     * Returns all HTTP request parts, provided that request is of type <code>multipart/form-data</code>. If there are no parts, an empty collection is
     * returned.
     *
     * @return all HTTP request parts.
     * @throws NullPointerException When faces context is unavailable.
     * @throws FacesException Whenever something fails at servlet or I/O level. The caller should preferably not catch it, but just let it go. The
    ...
    /**
     * Returns all HTTP request parts associated with the given name, provided that request is of type <code>multipart/form-data</code>. If there are no parts,
    ...
    /**
     * Returns the HTTP request context path. It's the webapp context name, with a leading slash. If the webapp runs on context root, then it returns an empty
     * string.
     * <p>
    ...
    /**
     * Returns the HTTP request servlet path. If Faces is prefix mapped (e.g. <code>/faces/*</code>), then this returns the whole prefix mapping (e.g.
     * <code>/faces</code>). If Faces is suffix mapped (e.g. <code>*.xhtml</code>), then this returns the whole part after the context path, with a leading
     * slash.
     * <p>
     ...
     * <code>*.xhtml</code>), then this returns <code>null</code>. If MultiViews is enabled, then this returns the part after the mapped view, if any.
     * <p>
    ...
    /**
     * Returns the HTTP request URI. This is the part after the domain in the request URL, including the leading slash. This does not include the request query
     * string.
     * <p>
    ...
    /**
     * Returns the HTTP request query string. This is the part after the <code>?</code> in the request URL as the enduser sees in browser address bar.
     * <p>
     ...
     * opposed to {@link #getRequestParameterValuesMap()}, which contains both the request URL (GET) parameters and the request body (POST) parameters. This is
    ...
    /**
     * Returns the HTTP request URI with query string. This is the part after the domain in the request URL, including the leading slash and the request query
     * string.
     * <p>
    ...
    /**
     * Returns the Internet Protocol (IP) address of the client that sent the request. This will first check the <code>X-Forwarded-For</code> request header and
     * if it's present, then return its first IP address, else just return {@link HttpServletRequest#getRemoteAddr()} unmodified.
     * <p>
    ...
    /**
     * Returns the HTTP response buffer size. If Facelets is used and the <code>jakarta.faces.FACELETS_BUFFER_SIZE</code> context parameter is been set, then
     * it's the context parameter value which will be returned. Otherwise it returns the implementation independent default value, which is 1024 in Mojarra.
     *
    ...
    /**
     * Sets the HTTP response status code. You can use the constant field values of {@link HttpServletResponse} for this. For example,
     * <code>Faces.setResponseStatus(HttpServletResponse.SC_BAD_REQUEST)</code>.
     *
    ...
    /**
     * Sends a temporary (302) redirect to the given URL. If the given URL does <b>not</b> start with <code>http://</code>, <code>https://</code> or
     * <code>/</code>, then the request context path will be prepended, otherwise it will be the unmodified redirect URL. So, when redirecting to another page
     * in the same web application, always specify the full path from the context root on (which in turn does not need to start with <code>/</code>).
     *
     * <pre>
    ...
    /**
     * Sends a permanent (301) redirect to the given URL. If the given URL does <b>not</b> start with <code>http://</code>, <code>https://</code> or
     * <code>/</code>, then the request context path will be prepended, otherwise it will be the unmodified redirect URL. So, when redirecting to another page
     * in the same web application, always specify the full path from the context root on (which in turn does not need to start with <code>/</code>).
     *
     * <pre>
    ...
    /**
     * Refresh the current page by a GET request, maintaining the query string. This basically sends a temporary (302) redirect to current request URI, with the
     ...
     * originally set values, with fall back to "text/html" and "UTF-8". This also resets the Faces partial response.
     *
    ...
    /**
     * Sends a HTTP response error with the given status and message. This will end up in either a custom <code>&lt;error-page&gt;</code> whose
     * <code>&lt;error-code&gt;</code> matches the given status, or in a servlet container specific default error page if there is none. The message will be
     * available in the error page as a request attribute with name <code>jakarta.servlet.error.message</code>. The {@link FacesContext#responseComplete()} will
     * implicitly be called after sending the error.
     *
     * @param status The HTTP response status which is supposed to be in the range 4nn-5nn. You can use the constant field values of {@link HttpServletResponse}
     * for this.
    ...
    /**
     * Returns whether the response is already committed. That is, when the response headers and a part of the response body has already been sent to the
     * client. This is usually a point of no return and you can't change the response anymore.
     *
    ...
    /**
     * Resets the current response. This will clear any headers which are been set and any data which is written to the response buffer which isn't committed
     * yet.
     *
     * @throws NullPointerException When faces context is unavailable.
    ...
    /**
     * Signals Faces that, as soon as the current phase of the lifecycle has been completed, control should be passed to the Render Response phase, bypassing
     * any phases that have not been executed yet.
     *
     * @throws NullPointerException When faces context is unavailable.
    ...
    /**
     * Returns <code>true</code> if we're currently in the render response phase. This explicitly checks the current phase ID instead of
     * {@link FacesContext#getRenderResponse()} as the latter may unexpectedly return false during a GET request when <code>&lt;f:viewParam&gt;</code> is been
     * used.
     * <p>
    ...
    /**
     * Signals Faces that the response for this request has already been generated (such as providing a file download), and that the lifecycle should be
    ...
    /**
     * Perform programmatic login for container managed FORM based authentication. Note that configuration is container specific and unrelated to Faces. Refer
     * the documentation of the servletcontainer using the keyword "realm".
     *
     * @param username The login username.
     * @param password The login password.
     * @throws NullPointerException When faces context is unavailable.
     * @throws ServletException When the login is invalid, or when container managed FORM based authentication is not enabled.
    ...
    /**
     * Trigger the default container managed authentication mechanism on the current request. It expects the username and password being available as
     * predefinied request parameters on the current request and/or a custom JASPIC implementation.
     *
    ...
    /**
     * Perform programmatic logout for container managed FORM based authentication. Note that this basically removes the user principal from the session. It's
     * however better practice to just invalidate the session altogether, which will implicitly also remove the user principal. Just invoke
     * {@link #invalidateSession()} instead. Note that the user principal is still present in the response of the current request, it's therefore recommend to
     * send a redirect after {@link #logout()} or {@link #invalidateSession()}. You can use {@link #redirect(String, Object...)} for this.
     *
     * @throws NullPointerException When faces context is unavailable.
    ...
    /**
     * Add a cookie with given name, value and maxage to the HTTP response. The cookie value will implicitly be URL-encoded with UTF-8 so that any special
     ...
     * @param maxAge The maximum age of the cookie, in seconds. If this is <code>0</code>, then the cookie will be removed. Note that the name and path must be
     * exactly the same as it was when the cookie was created. If this is <code>-1</code> then the cookie will become a session cookie and thus live as long as
     ...
     * @param name The cookie name.
     * @param value The cookie value.
     * @param path The cookie path. If this is <code>/</code>, then the cookie is available in all pages of the webapp. If this is
     * <code>/somespecificpath</code>, then the cookie is only available in pages under the specified path.
     * @param maxAge The maximum age of the cookie, in seconds. If this is <code>0</code>, then the cookie will be removed. Note that the name and path must be
     * exactly the same as it was when the cookie was created. If this is <code>-1</code> then the cookie will become a session cookie and thus live as long as
     * the established HTTP session.
     * @throws NullPointerException When faces context is unavailable.
     ...
     * @param name The cookie name.
     * @param value The cookie value.
     * @param domain The cookie domain. You can use <code>.example.com</code> (with a leading period) if you'd like the cookie to be available to all subdomains
     * of the domain. Note that you cannot set it to a different domain. Defaults to {@link #getRequestHostname()} when <code>null</code>.
     * @param path The cookie path. If this is <code>/</code>, then the cookie is available in all pages of the webapp. If this is
     * <code>/somespecificpath</code>, then the cookie is only available in pages under the specified path.
     * @param maxAge The maximum age of the cookie, in seconds. If this is <code>0</code>, then the cookie will be removed. Note that the name and path must be
     * exactly the same as it was when the cookie was created. If this is <code>-1</code> then the cookie will become a session cookie and thus live as long as
    ...
    /**
     * Add a cookie with given name, value, domain, path, maxage and HttpOnly flag to the HTTP response. The cookie value will implicitly be URL-encoded with
     * UTF-8 so that any special characters can be stored. The cookie will implicitly be set to secure when the current request is a HTTPS request.
     *
     * @param name The cookie name.
     * @param value The cookie value.
     * @param domain The cookie domain. You can use <code>.example.com</code> (with a leading period) if you'd like the cookie to be available to all subdomains
    ...
    /**
     * Add a cookie with given name, value, domain, path, maxage, HttpOnly flag and custom attributes to the HTTP response. The cookie value will implicitly be
     * URL-encoded with UTF-8 so that any special characters can be stored. The cookie will implicitly be set to secure when the current request is a HTTPS
     * request.
     *
     * @param name The cookie name.
     * @param value The cookie value.
     ...
     * the established HTTP session.
     * @param httpOnly When set to true, then JavaScript is not allowed to manipulate the cookie.
     * @param attributes Any custom attributes you'd like to add to the cookie, such as <code>SameSite:None</code>. Any key whose name matches a predefined
     * cookie attribute name, such as <code>domain</code>, <code>path</code>, <code>HttpOnly</code>, etc, will override them. If the underlying Faces and/or
     * Servlet implementation does not allow a certain custom attribute, then it will throw {@link IllegalArgumentException}.
     * @throws NullPointerException When faces context is unavailable.
     * @throws UnsupportedOperationException When this platform does not support UTF-8.
     * @throws IllegalArgumentException When a custom attribute is specified which is not supported by the underlying Faces and/or Servlet implementation. As of
     * now, it's only supported when running minimally Faces 4 on on top of minimally Servlet 6.
     * @see ExternalContext#addResponseCookie(String, String, Map)
     * @since 4.0
     */
    ...
    /**
     * Returns the HTTP session and creates one if one doesn't exist and <code>create</code> argument is <code>true</code>, otherwise don't create one and
    ...
    /**
     * Returns whether the HTTP session has been created for the first time in the current request. This returns also <code>false</code> when there is no means
     * of a HTTP session.
     *
     * @return <code>true</code> if the HTTP session has been created for the first time in the current request, otherwise <code>false</code>.
     * @throws NullPointerException When faces context is unavailable.
     * @see ExternalContext#getSession(boolean)
    ...
    /**
     * Returns the time of the previous request associated with the current HTTP session, measured in epoch time. This implicitly creates the session if one
     * doesn't exist.
     *
    ...
    /**
     * Sets the HTTP session timeout in seconds. A value of 0 or less means that the session should never timeout. This implicitly creates the session if one
     * doesn't exist.
     *
    ...
    /**
     * Returns the application initialization parameter map. This returns the parameter name-value pairs of all <code>&lt;context-param&gt;</code> entries in in
     * <code>web.xml</code>.
     *
    ...
    /**
     * Returns the application initialization parameter. This returns the <code>&lt;param-value&gt;</code> of a <code>&lt;context-param&gt;</code> in
     * <code>web.xml</code> associated with the given <code>&lt;param-name&gt;</code>.
     *
     * @param name The application initialization parameter name.
     * @return The application initialization parameter value associated with the given name, or <code>null</code> if there is none.
     * @throws NullPointerException When faces context is unavailable.
     ...
     * @return The application initialization parameter value associated with the given name, or <code>defaultValue</code> if there is none.
     * @throws NullPointerException When faces context is unavailable.
     * @see ExternalContext#getInitParameter(String)
     * @since 3.1
     */
    ...
    /**
     * Returns the mime type for the given file name. The mime type is determined based on file extension and configureable by <code>&lt;mime-mapping&gt;</code>
     * entries in <code>web.xml</code>. When the mime type is unknown, then a default of <code>application/octet-stream</code> will be returned.
     *
    ...
    /**
     * Returns a URL for an application resource mapped to the specified path, if it exists; otherwise, return <code>null</code>.
     *
    ...
    /**
     * Returns an input stream for an application resource mapped to the specified path, if it exists; otherwise, return <code>null</code>.
     *
     * @param path The application resource path to return an input stream for.
     * @return An input stream for an application resource mapped to the specified path.
     * @throws NullPointerException When faces context is unavailable.
    ...
    /**
     * Returns the absolute disk file system path representation of the given web content path. This thus converts the given path of a web content resource
     * (e.g. <code>/index.xhtml</code>) to an absolute disk file system path (e.g. <code>/path/to/server/work/folder/some.war/index.xhtml</code>) which can then
     * be used in {@link File}, {@link FileInputStream}, etc.
     * <p>
     * Note that this will return <code>null</code> when the WAR is not expanded into the disk file system, but instead into memory. If all you want is just an
     * {@link InputStream} of the web content resource, then better use {@link #getResourceAsStream(String)} instead.
     * <p>
     * Also note that it wouldn't make sense to modify or create files in this location, as those changes would get lost anyway when the WAR is redeployed or
     * even when the server is restarted. This is thus absolutely not a good location to store for example uploaded files.
     *
     ...
     * @param <T> The expected return type.
     * @param name The request scope attribute name.
     * @return The request scope attribute value previously associated with the given name, or <code>null</code> if there is no such attribute.
     * @throws NullPointerException When faces context is unavailable.
     * @throws ClassCastException When <code>T</code> is of wrong type.
     * @see ExternalContext#getRequestMap()
     * @since 1.1
    ...
    /**
     * Returns the flash scope. Note that <code>Flash</code> implements <code>Map&lt;String, Object&gt;</code>, so you can just treat it like a
     ...
     * @param <T> The expected return type.
     * @param name The flash scope attribute name.
     * @return The flash scope attribute value previously associated with the given name, or <code>null</code> if there is no such attribute.
     * @throws NullPointerException When faces context is unavailable.
     * @throws ClassCastException When <code>T</code> is of wrong type.
     * @see ExternalContext#getFlash()
     * @since 1.1
     ...
     * @param <T> The expected return type.
     * @param name The view scope attribute name.
     * @return The view scope attribute value previously associated with the given name, or <code>null</code> if there is no such attribute.
     * @throws NullPointerException When faces context is unavailable.
     * @throws ClassCastException When <code>T</code> is of wrong type.
     * @see UIViewRoot#getViewMap()
     * @since 1.1
     ...
     * @param <T> The expected return type.
     * @param name The session scope attribute name.
     * @return The session scope attribute value previously associated with the given name, or <code>null</code> if there is no such attribute.
     * @throws NullPointerException When faces context is unavailable.
     * @throws ClassCastException When <code>T</code> is of wrong type.
     * @see ExternalContext#getSessionMap()
     * @since 1.1
     ...
     * @param <T> The expected return type.
     * @param name The application scope attribute name.
     * @return The application scope attribute value previously associated with the given name, or <code>null</code> if there is no such attribute.
     * @throws NullPointerException When faces context is unavailable.
     * @throws ClassCastException When <code>T</code> is of wrong type.
     * @see ExternalContext#getApplicationMap()
     * @since 1.1
    ...
    /**
     * Send the given file to the response. The file name will be derived from {@link File#getName()}. The content type will be determined based on file name.
     * The content length will be set to the length of the file. The {@link FacesContext#responseComplete()} will implicitly be called after successful
     * streaming.
     *
    ...
    /**
     * Send the given file to the response. The content type will be determined based on file name. The content length will be set to the length of the file.
    ...
    /**
     * Send the given path as a file to the response. The file will be derived from {@link Path#toFile()} and then the file name will be derived from
     * {@link File#getName()}. The content type will be determined based on file name. The content length will be set to the length of the file. The
    ...
    /**
     * Send the given path as a file to the response. The file will be derived from {@link Path#toFile()}. The content type will be determined based on file
     * name. The content length will be set to the length of the file. The {@link FacesContext#responseComplete()} will implicitly be called after successful
     * streaming.
     *
     * @param path The path to be sent as a file to the response.
     * @param filename The file name which should appear in content disposition header.
    ...
    /**
     * Send the given byte array as a file to the response. The content type will be determined based on file name. The content length will be set to the length
     * of the byte array. The {@link FacesContext#responseComplete()} will implicitly be called after successful streaming.
     *
    ...
    /**
     * Send the given input stream as a file to the response. The content type will be determined based on file name. The content length may not be set because
     * that's not predictable based on input stream. The client may receive a download of an unknown length and thus the download progress may be unknown to the
     * client. Only if the input stream is smaller than the default buffer size, then the content length will be set. The {@link InputStream#close()} will
     * implicitly be called after streaming, regardless of whether an exception is been thrown or not. The {@link FacesContext#responseComplete()} will
    ...
    /**
     * Send a file to the response whose content is provided via given output stream callback. The content type will be determined based on file name. The
     * content length will not be set because that would require buffering the entire file in memory or temp disk. The client may receive a download of an
     * unknown length and thus the download progress may be unknown to the client. If this is undesirable, write to a temp file instead and then use
     * {@link Faces#sendFile(File, boolean)}. The {@link FacesContext#responseComplete()} will implicitly be called after successful streaming.
     *
     * @param filename The file name which should appear in content disposition header.
     * @param attachment Whether the file should be provided as attachment, or just inline.
