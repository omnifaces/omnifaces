...
public class SourceMapResourceHandler extends DefaultResourceHandler {
	...
	public Resource decorateResource(Resource resource, String resourceName, String libraryName) {
		...
		if (cachedSourceMap == null && !sourceMap.isEmpty()) {
			SOURCE_MAPS.put(resourceIdentifier, sourceMap); // Only resources which actually have a source map are cached, so that arbitrary resource names cannot grow the cache unbounded.
		}
