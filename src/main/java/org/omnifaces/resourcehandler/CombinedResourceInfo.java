...
public final class CombinedResourceInfo {
    ...
    private static CombinedResourceInfo create(String id, Set<ResourceIdentifier> resourceIdentifiers) {
        var info = new CombinedResourceInfo(id, Collections.unmodifiableSet(resourceIdentifiers), true);
        CACHE.put(id, info);
        return info;
    }
    ...
    /**
     * Returns whether the ID of this combined resource info was issued by this deployment. A combined resource ID is unauthenticated, so an ID which was
     * reconstructed from a request may name any combinable resource and is not necessarily one this deployment would ever render.
     *
    ...
    /**
     * Returns whether all given resource identifiers represent combinable resources, i.e. stylesheets and scripts. A combined resource may only reference
     * stylesheet and script resources, never e.g. Facelets files or dynamic resources.
     *
    ...
    private static boolean isCombinable(Set<ResourceIdentifier> resourceIdentifiers) {
        for (var resourceIdentifier : resourceIdentifiers) {
            var name = resourceIdentifier.getName();
