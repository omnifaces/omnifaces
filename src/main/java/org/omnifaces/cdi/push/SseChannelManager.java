...
public class SseChannelManager extends PushChannelManager {
     ...
     * exists yet, then this bypasses this {@link SessionScoped} bean, so that <code>&lt;o:sse&gt;</code> stays usable on stateless views.
     *
     ...
     * @return The SSE channel identifier.
    ...
    static String register(FacesContext context, String tagName, String channel, String scope, Serializable user) {
        ...
        return resolvedScope == Scope.APPLICATION && !hasSession(context)
            ? registerApplicationScopedChannel(channel, APPLICATION_SCOPE, getReference(SseSessionManager.class))
