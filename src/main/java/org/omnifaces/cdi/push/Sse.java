...
/**
 * <p>
 * The <code>&lt;o:sse&gt;</code> is an {@link UIComponent} which opens an one-way (server to client) SSE (Server-Sent Events) based push connection in client
 * side which can be reached from server side via {@link PushContext} interface injected in any CDI/container managed artifact via
 * <code>&#64;</code>{@link Push}<code>(type=SSE)</code> annotation.
 *
 * <h2 id="prerequisites"><a href="#prerequisites">Prerequisites</a></h2>
 * <p>
 * All servlet filters mapped on {@code /*} must have {@code @WebFilter(asyncSupported=true)} or {@code web.xml <async-supported>true</async-supported>} for SSE
 * to work. In case that's not possible for your filter (e.g because it relies on some request/thread-specific state after invoking {@code chain.doFilter()}
 * such as DB connection, locked/shared resource, {@code ThreadLocal}, etc), then you'll need to map it on a more specific URL-pattern excluding
 * {@value PushContext#SSE_URI_PREFIX} and create yet another filter on <code>/*</code> which forwards to that filter when the request path does not match
 * {@value PushContext#SSE_URI_PREFIX}.
 * <p>
 * The server (and proxy, if any) must also support HTTP/2 and have it enabled. Otherwise it will fall back to HTTP/1.1 which limits concurrent connections per
 * origin (e.g. Chrome limits to 6). In such case, multiple SSE connections across multiple tabs may hit this limit and queue further HTTP requests (including
 * Ajax requests!), which has the consequence that {@code <f:ajax>} will simply stop working. With HTTP/2, SSE streams are multiplexed over a single TCP
 * connection.
 *
 * <h2 id="sse-vs-websocket"><a href="#sse-vs-websocket">SSE vs WebSocket</a></h2>
 * <p>
 * Both <code>&lt;o:sse&gt;</code> and <code>&lt;o:socket&gt;</code> provide one-way (server to client) push. The table below summarizes the key differences to
 * help you choose the right transport.
 *
 * <table>
 * <caption>SSE (<code>&lt;o:sse&gt;</code>) vs WebSocket (<code>&lt;o:socket&gt;</code>)</caption> <thead>
 * <tr>
 * <th></th>
 * <th>SSE (<code>&lt;o:sse&gt;</code>)</th>
 * <th>WebSocket (<code>&lt;o:socket&gt;</code>)</th>
 * </tr>
 * </thead> <tbody>
 * <tr>
 * <td><b>Transport</b></td>
 * <td>Plain HTTP</td>
 * <td>WebSocket protocol upgrade</td>
 * </tr>
 * <tr>
 * <td><b>Dependencies</b></td>
 * <td>None (async servlet)</td>
 * <td>Requires <code>jakarta.websocket</code></td>
 * </tr>
 * <tr>
 * <td><b>Proxy/CDN/firewall</b></td>
 * <td>Works through HTTP infrastructure</td>
 * <td>May be blocked by proxies or corporate firewalls</td>
 * </tr>
 * <tr>
 * <td><b>Reconnect</b></td>
 * <td>Built-in (<code>EventSource</code> API)</td>
 * <td>Manual (with cumulative backoff)</td>
 * </tr>
 * <tr>
 * <td><b>HTTP/2 multiplexing</b></td>
 * <td>Yes (shares TCP connection)</td>
 * <td>No (separate TCP connection)</td>
 * </tr>
 * <tr>
 ...
 * </tr>
 * <tr>
 * <td><b>CDI events</b></td>
 * <td>{@link SseEvent} on open/close/switch</td>
 * <td>{@link SocketEvent} on open/close/switch</td>
 * </tr>
 * <tr>
 * <td><b>Dynamic connect/disconnect</b></td>
 * <td>Not available</td>
 * <td><code>connected</code> attribute</td>
 * </tr>
 * </tbody>
 * </table>
 * <p>
 * <b>When to prefer SSE:</b> for typical unidirectional push use cases (notifications, live updates, dashboards) where HTTP infrastructure compatibility and
 * simplicity are important. SSE is the simpler and more robust choice for most server-to-client push scenarios.
 * <p>
 * <b>When to prefer WebSocket:</b> when you need dynamic connect/disconnect via the {@code connected} attribute, or when your server does not support HTTP/2.
 *
 *
 ...
 * <p>
 * Declare <strong><code>&lt;o:sse&gt;</code></strong> tag in the Faces view with at least a <strong><code>channel</code></strong> name and an
 ...
 * </ul>
 * <p>
 * The SSE connection is automatically established when the page loads and will auto-reconnect when the connection is lost due to e.g. a network error or server
 * restart. This is a built-in feature of the browser's <a href="https://developer.mozilla.org/en-US/docs/Web/API/EventSource"><code>EventSource</code></a> API.
 * The SSE connection will be implicitly closed once the document is unloaded (e.g. navigating away, close of browser window/tab, etc).
 * <p>
 ...
 * <p>
 * By default the SSE channel is <code>application</code> scoped, i.e. any view/session throughout the web application having the same SSE channel open will
 * receive the same push message. The push message can be sent by all users and the application itself. This is useful for application-wide feedback triggered
 * by site itself such as real time updates of a certain page (e.g. site-wide statistics, top100 lists, stock updates, etc).
 * <p>
 ...
 * invaliding it &mdash; which is in turn poor security practice). If in such case a session scoped channel is reused, undefined behavior may occur when
 ...
 * <p>
 * When the <code>user</code> attribute is an EL expression and it changes during an ajax request, then the SSE user will be actually switched, even though you
 * did not cover the <code>&lt;o:sse&gt;</code> component in any ajax render/update. So make sure the value is tied to at least a view scoped property in case
 * you intend to control it during the view scope.
 * <p>
 ...
 * <p>
 * The optional <strong><code>onopen</code></strong> JavaScript listener function can be used to listen on open of an SSE connection in client side. It will be
 ...
 * </pre>
 * <p>
 * The optional <strong><code>onerror</code></strong> JavaScript listener function can be used to listen on a connection error whereby the browser will
 ...
 * <ul>
 * <li><code>code</code>: synthetic HTTP-based close code as integer. Currently this is always <code>500</code> (connection error, browser will
 * auto-reconnect).</li>
 * <li><code>channel</code>: the channel name, useful in case you intend to have a global listener.</li>
 * <li><code>event</code>: the raw <a href="https://developer.mozilla.org/en-US/docs/Web/API/EventSource/error_event"> <code>Event</code></a> instance from the
 * <code>EventSource</code> API. Note that unlike WebSocket's <code>CloseEvent</code>, the SSE error event does not carry a close code or reason.</li>
 * </ul>
 * <p>
 * The optional <strong><code>onclose</code></strong> JavaScript listener function can be used to listen on the final close of an SSE connection. This will be
 ...
 * <ul>
 * <li><code>code</code>: synthetic HTTP-based close code as integer. If this is <code>-1</code>, then the <code>EventSource</code> API is simply not
 * <a href="https://caniuse.com/eventsource">supported</a> by the client. If this is <code>200</code>, then the server explicitly closed the connection (e.g.
 * due to an expired session or view). If this is <code>204</code>, then the client explicitly closed the connection via
 * <code>OmniFaces.Push.close(channel)</code>. If this is <code>404</code>, then the channel is unknown (e.g. randomly guessed or spoofed by endusers or
 * manually reconnected after the session is expired).</li>
 * <li><code>channel</code>: the channel name, useful in case you intend to have a global listener.</li>
 * <li><code>event</code>: the raw <a href="https://developer.mozilla.org/en-US/docs/Web/API/EventSource/error_event"> <code>Event</code></a> instance, if
 * available. This is present when the server closed the connection (codes <code>200</code> and <code>404</code>), but absent when the <code>EventSource</code>
 * API is not supported (code <code>-1</code>) or the client explicitly closed the connection (code <code>204</code>).</li>
 * </ul>
 *
 *
 * <h2 id="events-server"><a href="#events-server">Events (server)</a></h2>
 * <p>
 * When an SSE connection has been opened, a new CDI <strong>{@link SseEvent}</strong> will be fired with <strong><code>&#64;</code>{@link Opened}</strong>
 * qualifier. When the <code>user</code> attribute of the <code>&lt;o:sse&gt;</code> changes, a new CDI <strong>{@link SseEvent}</strong> will be fired with
 ...
 * </pre>
 * <p>
 * You could take the opportunity to send another push message to an application scoped channel, e.g. "User X has been logged in" (or out) when a session scoped
 * channel is opened (or closed).
 *
 *
 * <h2 id="security"><a href="#security">Security considerations</a></h2>
 * <p>
 * If the SSE channel is declared in a page which is only restricted to logged-in users with a specific role, then you may want to add the URL of the SSE
 * connection to the set of restricted URLs.
 * <p>
 ...
 * </pre>
 * <p>
 * As extra security, particularly for those public channels which can't be restricted by security constraints, the <code>&lt;o:sse&gt;</code> will register all
 * so far declared channels in the current HTTP session, and any incoming SSE connection request will be checked whether it matches a registered channel in the
 * current HTTP session. In case the channel is unknown (e.g. randomly guessed or spoofed by endusers or manually reconnected after the session is expired),
 * then the SSE connection will immediately be completed. Also, when the HTTP session gets destroyed, all session and view scoped channels which are still open
 * will explicitly be closed from server side. Only application scoped channels remain open and are still reachable from server end even when the session or
 ...
 * which can only be identified by a HTTP request). A view and session scoped push channel would also not work, so the push channel really needs to be
 ...
 * </pre>
 * <p>
 * This would be the only way in case you intend to asynchronously send a message to a view or session scoped push channel, and/or want to pass something from
 ...
 * client is connected to, then it won't reach the client. One solution is to activate and configure a JMS topic in the server configuration, trigger the push
