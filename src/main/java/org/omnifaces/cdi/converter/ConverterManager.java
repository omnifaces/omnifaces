...
/**
 * <p>
 * The <code>@FacesConverter</code> is by default not eligible for dependency injection by <code>@Inject</code> nor <code>@EJB</code>. It that only when the
 * <code>managed=true</code> attribute is set. But this doesn't support setting custom attributes. OmniFaces solves this by implicitly making all
 * {@link FacesConverter} instances eligible for dependency injection <strong>without any further modification</strong>. In order to utilize OmniFaces managed
 * converter, simply remove the Faces native <code>managed=true</code> attribute.
 * <p>
 ...
 * <p>
 * In case you have a {@link FacesConverter} annotated class extending another {@link FacesConverter} annotated class which in turn extends a standard
 * converter, then you may with <code>bean-discovery-mode="all"</code> face an {@link AmbiguousResolutionException}. This can be solved by placing
...
public class ConverterManager {
    ...
    @PostConstruct
    public void init() {
        dateTimeConverterDefaultTimeZone = parseBoolean(getInitParameter(DATETIMECONVERTER_DEFAULT_TIMEZONE_IS_SYSTEM_TIMEZONE_PARAM_NAME))
            ? TimeZone.getDefault()
            : null;
    }
