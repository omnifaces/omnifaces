...
/**
 * <p>
 * The CDI annotation <code>&#64;</code>{@link Param} allows you to inject, convert and validate a HTTP request or path parameter in a CDI managed bean.
 * <p>
 * For HTTP request parameters it's basically like <code>&lt;f:viewParam&gt;</code>, but with the major difference that the injected parameter is directly
 * available during {@link PostConstruct}, allowing a much easier way of processing without the need for a <code>&lt;f:event type="preRenderView"&gt;</code> or
 ...
 * </pre>
 * <p>
 * If conversion or validation fails, <code>null</code> is injected if the injection target is <b>NOT</b> {@link ParamValue}. Otherwise a {@link ParamValue}
 * instance is injected, but it will contain a <code>null</code> value. In both cases, the conversion and validation messages (if any) will be set in the Faces
 * context then, and {@link FacesContext#isValidationFailed()} will return <code>true</code>.
 *
...
public @interface Param {
    ...
    /**
     * (Optional/Required) The converter to be used for converting the parameter to the type that is to be injected. Optional if the target type is String,
     * otherwise required.
     * <p>
     * A converter can be specified in 3 ways:
     * <ol>
     * <li>A string value representing the <em>converter-id</em> as used by {@link jakarta.faces.application.Application#createConverter(String)}
    ...
    /**
     * (Optional) Flag indicating if this parameter is required (must be present) or not. The required check is done after conversion and before validation. A
     * value is said to be not present if it turns out to be empty according to the semantics of {@link Utils#isEmpty(Object)}.
     *
     ...
     * <ol>
     * <li>A string value representing the <em>validator-id</em> as used by {@link jakarta.faces.application.Application#createValidator(String)}
     ...
     * </ol>
     * <p>
     * If this attribute is specified in addition to {@link Param#validatorClasses()} then the validators from both attributes will be added to the final
     * collection of validators. The validators from this attribute will however be called first.
     *
    ...
    /**
     * (Optional) Class of the converter to be used for converting the parameter to the type that is to be injected. This is ignored when {@link #converter()}
     * is specified.
     *
    ...
    /**
     * (Optional) Class of one ore more validators to be used for validating the (converted) parameter. These will run <i>after</i> the ones specified in
     * {@link #validators()}.
     *
     * @return The validator classes used to validate the (converted) parameter.
     */
    @Nonbinding
    Class<? extends Validator>[] validatorClasses() default {};
     ...
     * <p>
     * For each attribute the converter instance should have a writable JavaBeans property with the same name. The value can be a string literal or an EL
     * expression. String literals are coerced if necessary if there's a {@link PropertyEditor} available (the JDK provides these for the primitive types and
     * their corresponding boxed types).
     * <p>
     ...
     */
    @Nonbinding
    Attribute[] converterAttributes() default {};
     ...
     * <p>
     * For each attribute the validator instances should have a writable JavaBeans property with the same name. The value can be a string literal or an EL
     * expression. String literals are coerced if necessary if there's a {@link PropertyEditor} available (the JDK provides these for the primitive types and
     * their corresponding boxed types).
     * <p>
     ...
     */
    @Nonbinding
    Attribute[] validatorAttributes() default {};

    /**
     * (Optional) A message that will be used if conversion fails instead of the message set by the converter.
     * <p>
     * The value for which conversion failed is available as <code>{0}</code>. The label associated with this parameter value (see the {@link Param#label()}
     * attribute) is available as <code>{1}</code>.
     *
     * @return The error message to be used when the {@link #converter()} or {@link #converterClass()} fail.
     */
    @Nonbinding
    String converterMessage() default "";

    /**
     * (Optional) A message that will be used if validation fails instead of the message set by the validator(s).
     * <p>
     * The value for which validation failed is available as <code>{0}</code>. The label associated with this parameter value (see the {@link Param#label()}
     * attribute) is available as <code>{1}</code>.
     *
    ...
    /**
     * (Optional) A message that will be used if a non-empty value is submitted instead of the default message associated with the {@link RequiredValidator}.
     * <p>
     * The (empty) value for which the required check failed is available as <code>{0}</code>. (this will be either null or the empty string) The label
     * associated with this parameter value (see the {@link Param#label()} attribute) is available as <code>{1}</code>.
     *
     ...
     * <p>
     * <b>NOTE:</b> bean validation at the moment (OmniFaces 1.6) is done against the {@link ParamValue} that is injected. In many cases this will be of limited
     * use. We hope to directly inject the converted type in OmniFaces 1.7 and then bean validation will make more sense.
     * <p>
     * If <code>true</code> no bean validation will be attempted. If <code>false</code> (the default) no specific action is taken, and it will depend on the
     * availability of bean validation and the global {@link BeanValidator#DISABLE_DEFAULT_BEAN_VALIDATOR_PARAM_NAME} setting whether bean validation is
     * attempted or not.
     *
     * @return Whether to disable bean validation or not.
     */
    @Nonbinding
    boolean disableBeanValidation() default false;

    /**
     * (Optional) Flag that overrides the global {@link BeanValidator#DISABLE_DEFAULT_BEAN_VALIDATOR_PARAM_NAME} setting.
     * <p>
     * If <code>true</code> bean validation will be performed for this instance (given that bean validation is available) despite it globally being disabled. If
     * <code>false</code> (the default) no specific action is taken.
     *
     * @return Whether to override that Faces bean validation is globally disabled.
     */
    @Nonbinding
    boolean overrideGlobalBeanValidationDisabled() default false;
