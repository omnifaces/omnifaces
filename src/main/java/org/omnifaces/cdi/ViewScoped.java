 ...
 * <p>
 * In standard Faces, the <code>&#64;</code>{@link PreDestroy} annotated method on a view scoped bean is only invoked when the session expires. However, there
 * may be cases when it's desirable to immediately destroy a view scoped bean as well when the browser <code>unload</code> event is invoked. I.e. when the user
 * navigates away by GET, or closes the browser tab/window. The standard Faces view scope does not support this. Since OmniFaces 2.2, this CDI view scope
 * annotation will guarantee that the <code>&#64;PreDestroy</code> annotated method is also invoked on browser unload. This trick is done by
 * <code>navigator.sendBeacon</code>. For browsers not supporting <code>navigator.sendBeacon</code>, it will fallback to a synchronous XHR request.
 * <p>
 * Since OmniFaces 2.3, the unload has been further improved to also physically remove the associated Faces view state from Faces implementation's internal LRU
 * map in case of server side state saving, hereby further decreasing the risk at <code>ViewExpiredException</code> on the other views which were created/opened
 * earlier. As side effect of this change, the <code>&#64;PreDestroy</code> annotated method of any standard Faces view scoped beans referenced in the same view
 * as the OmniFaces CDI view scoped bean will also guaranteed be invoked on browser unload.
 * <p>
 * Since OmniFaces 2.6, this annotation got a new attribute: <code>saveInViewState</code>. When using client side state saving, this attribute can be set to
 ...
 * </pre>
 * <p>
 * Please note that the bean <strong>must</strong> implement {@link Serializable}, otherwise the CDI implementation will throw an exception about the bean not
 * being passivation capable.
 * <p>
 * Under the covers, CDI managed beans with this scope are via {@link ViewScopeManager} by default stored in the session scope by an {@link UUID} based key
 * which is referenced in Faces own view map as available by {@link UIViewRoot#getViewMap()}. They are not stored in the Faces view state itself as that would
 * be rather expensive in case of client side state saving.
 * <p>
 ...
 * </pre>
 * <p>
 * It's very important that you understand that this setting has potentially a major impact in the size of the Faces view state, certainly when the view scoped
 * bean instance holds "too much" data, such as a collection of entities for a data table, and that such beans will in fact <strong>never</strong> expire as
 * they are stored entirely in the <code>jakarta.faces.ViewState</code> hidden input field in the HTML page. Moreover, the <code>&#64;</code>{@link PreDestroy}
 * annotated method on such bean will explicitly <strong>never</strong> be invoked, even not on an unload as it's quite possible to save or cache the page
 * source and re-execute it at a (much) later moment.
 * <p>
 * It's therefore strongly recommended to use this setting only on a view scoped bean instance which is exclusively used to keep track of the dynamically
 * controlled form state, such as <code>disabled</code>, <code>readonly</code> and <code>rendered</code> attributes which are controlled by ajax events.
 * <p>
 * This setting is NOT recommended when using server side state saving. It has basically no effect and it only adds unnecessary serialization overhead. The
 * system will therefore throw an {@link IllegalStateException} on such condition.
 *
 * <h2>Configuration</h2>
 * <p>
 * By default, the maximum number of active view scopes is hold in a LRU map in HTTP session with a default size equal to the first non-null value of the
 ...
 * </ul>
 * <p>
 * If none of those context parameters are present, then a default size of {@value org.omnifaces.cdi.viewscope.ViewScopeManager#DEFAULT_MAX_ACTIVE_VIEW_SCOPES}
 * will be used. When a view scoped bean is evicted from the LRU map, then its <code>&#64;PreDestroy</code> will also guaranteed to be invoked.
 * <p>
 ...
 * <p>
 * When the unload request has hit your servlet filter or authentication mechanism or whatever global listener/observer, and you would like to be able to detect
 * them, so that you can exclude them from the logic, then you can use {@link ViewScopeManager#isUnloadRequest(jakarta.servlet.http.HttpServletRequest)} or
 * {@link ViewScopeManager#isUnloadRequest(FacesContext)}, depending on whether the {@link FacesContext} is available in the current context. You should always
...
public @interface ViewScoped {
    ...
    /**
     * <p>
     * Sets whether to save the view scoped bean instance in Faces view state instead of in HTTP session. By default, concrete view scoped bean instances are
     * saved in HTTP session, also when client side state saving is enabled. This means, when the HTTP session expires, then the view scoped bean instances will
     * also implicitly expire and be newly recreated upon a request in a new session. This may be undesirable when using client side state saving as it's
     * intuitively expected that concrete view scoped beans are also saved in Faces view state.
     *
     ...
     */
    @Nonbinding
    boolean saveInViewState() default false;
