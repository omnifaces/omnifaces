 ...
 * <p>
 * A variable can be declared, which marks it as a tag attribute of that tag file invocation, or set, which is the
 * ordinary variable mapper contract as used by <code>&lt;c:set&gt;</code> and <code>&lt;ui:param&gt;</code>. Only a
 * declared variable is shielded from a nested tag file invocation, and only a declared variable also remembers the
 * value which it inherited at the moment of declaring, its boundary value.
 * <p>
 * A nested instance therefore treats an inherited value as not inherited when it equals either the declared value or
 * the boundary value of an enclosing instance. Both are looked up through the wrapped mapper under a prefixed name, so
 * that they are also found when the Faces implementation puts its own variable mapper in between, which Mojarra does as
 * soon as the nested tag file is passed an attribute of its own. The boundary value is what makes this work on MyFaces
 * as well, whose variable mapper is dynamic and answers for the tag file which is currently being applied.
 *
 * @author Arjan Tijms.
 * @since 2.1
 */
public class DelegatingVariableMapper extends VariableMapper {
	...
	/**
	 * Returns the value which this mapper inherits for the given name, or <code>null</code> when that value is merely
	 * the declaration of an enclosing mapper and therefore not inherited at all.
	 *
	...
	public ValueExpression resolveWrappedVariable(String name) {
		...
		if (Objects.equals(inheritedVariable, wrapped.resolveVariable(DECLARED_PREFIX + name))
	...
	/**
	 * Declares the given name as a tag attribute of the tag file invocation which this mapper represents, remembering
	 * the value which it inherits at this moment, so that a nested tag file invocation can recognize the declaration
	 * as not being its own.
	 *
	 * @param name Name of the variable.
