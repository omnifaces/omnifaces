...
/**
 * <strong>ValidateOrderLt</strong> validates if the values of the given <code>UIInput</code> components are in exactly
 * the same order from least to greatest without duplicates as specified in the <code>components</code> attribute.
 ...
 * @since 1.1
 */
@FacesComponent(ValidateOrderLt.COMPONENT_TYPE)
public class ValidateOrderLt extends ValidateOrder {

	// Public constants -----------------------------------------------------------------------------------------------

	/** The standard component type. */
	public static final String COMPONENT_TYPE = "org.omnifaces.component.validator.ValidateOrderLt";

	// Actions --------------------------------------------------------------------------------------------------------

	@Override
	protected <T extends Comparable<T>> boolean validateOrder(FacesContext context, List<UIInput> components, List<T> values) {
		return new ArrayList<T>(new TreeSet<T>(values)).equals(values);
	}
