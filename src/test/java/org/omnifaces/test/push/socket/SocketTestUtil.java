...
/**
 * Test helpers to perform the web socket handshake as a raw HTTP upgrade, so that the connecting HTTP session (i.e. the presence or absence of the
...
final class SocketTestUtil {
    ...
    static Socket sendHandshake(URL baseURL, String channelId, String sessionCookie) throws IOException {
        ...
        String path = baseURL.getPath().replaceFirst("/$", "") + PushContext.SOCKET_URI_PREFIX + "/" + channelId;
    ...
    /**
     * Reads and consumes the HTTP response head from the given input stream (leaving it positioned at any web socket frame that follows) and returns its status
     * code, i.e. 101 when the handshake is accepted, or -1 when the server did not return a status line.
     */
